...
const AppReducer = (state, action) => {
  switch (action.type) {
    ...
    case "clear":
      if (
        state.buttonInput === "MATH ERROR" ||
        state.buttonInput === "Syntax Error"
      ) {
        return {
          ...state,
          buttonInput: "",
        ...
        return {
          ...state,
          buttonInput: state.buttonInput.slice(0, state.buttonInput.length - 1),
          result: "",
        };
      }

    case "=":
      let myRegex = /[^0-9]$|[a-zA-z]+|^[*/%]+/gi;
      let operatorRegex = /[+\-*/%]{2,}/;
      ...
      if (myRegex.test(state.buttonInput)) {
        ...
        return {
          ...state,
          result: res % 1 === 0 ? res : res.toFixed(3),
        };
      }
